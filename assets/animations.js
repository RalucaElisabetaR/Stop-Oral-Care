...
document.addEventListener("DOMContentLoaded", function() {
  ...
  slideshowComponents.forEach(slideshowComponent => {
    ...
    const scrollSpeed = 1; // Speed of the auto-scroll

    const autoScrollFunction = () => {
      if (!isDragging) {
        if (slideshowComponent.scrollLeft >= slideshowComponent.scrollWidth - slideshowComponent.offsetWidth) {
          slideshowComponent.scrollLeft = 0;
        } else {
          slideshowComponent.scrollLeft += scrollSpeed;
        }
      }
      autoScroll = setTimeout(autoScrollFunction, 30); // 30 milliseconds for smoother control
    };
    ...
    slideshowComponent.addEventListener('mouseleave', () => {
      isDragging = false;
      autoScrollFunction(); // Restart auto-scroll
    });
