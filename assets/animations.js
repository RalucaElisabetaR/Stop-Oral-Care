...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const resetPosition = 50; // Reset position when nearing the end
  ...
  slideshowComponents.forEach(slideshowComponent => {
    ...
    const startAutoScroll = () => {
      autoScroll = setInterval(() => {
        if (slideshowComponent.scrollLeft >= (slideshowComponent.scrollWidth - slideshowComponent.offsetWidth - resetPosition)) {
          slideshowComponent.scrollLeft = 0;
        } else {
          slideshowComponent.scrollLeft += scrollSpeed;
        }
      }, 16);  // 16 milliseconds for smooth scrolling (about 60fps)
    };
    ...
    slideshowComponent.addEventListener('mousedown', () => {
      clearInterval(autoScroll);
    });

    // Resume auto-scrolling when user stops dragging
    slideshowComponent.addEventListener('mouseup', () => {
      startAutoScroll();
    });

    // Resume auto-scrolling when mouse leaves the component
    slideshowComponent.addEventListener('mouseleave', () => {
      startAutoScroll();
    });
  });
